...
jsbApp.pcards = document.getElementById('player');
jsbApp.dcards = document.getElementById('dealer');
...
jsbApp.phandtext = document.getElementById('player');
jsbApp.dhandtext = document.getElementById('dealer');
...
var newGame = function () {
    ...
    jsbApp.player.push(jsbApp.deck.pop());
    jsbApp.player.push(jsbApp.deck.pop());

    // check for player victory
    if (handTotal(jsbApp.player) === 21) {
        jsbApp.wins += 1;
        jsbApp.games += 1;
    ...
    jsbApp.dealer.push(jsbApp.deck.pop());
    jsbApp.dealer.push(jsbApp.deck.pop());

    // check for dealer victory    
    if (handTotal(jsbApp.dealer) === 21) {
        jsbApp.games += 1;
        jsbApp.losses += 1;
        jsbApp.gameStatus = 1; // to cause the dealer's hand to be drawn face up
        drawHands();
    ...
    drawHands();
    advise();
    jsbApp.buttonBox.classList.remove(""); // show hit/stay buttons
...
jsbApp.hitButton.addEventListener("click", function () {
    ...
    var handVal = handTotal(jsbApp.player);
    if (handVal > 21) {
        bust();
        advise();
        return;
...
jsbApp.stayButton.addEventListener("click", function stayLoop() {
    ...
    else if (jsbApp.gameStatus === 0) // i.e. stay was just pressed
    {
        ...
        var handVal = handTotal(jsbApp.dealer);
        ...
        if (handVal > 16 && handVal <= 21) // dealer stays and game resolves
        {
            ...
            var playerVal = handTotal(jsbApp.player);
...
var victory = function () {
    ...
    var playerTotal = handTotal(jsbApp.player);
    var dealerTotal = handTotal(jsbApp.dealer);
    if (playerTotal === 21) {
...
var advise = function () {
    ...
    var playerTotal = handTotal(jsbApp.player);
    var soft = softCheck(jsbApp.player);
